...
import { getRestaurant, getVisitsByRestaurant, deleteVisit, deletePhoto, updatePhoto } from '../lib/storage'
...
export default function RestaurantDetail() {
  ...
  // Tick every 30s so sealed countdowns update live
  useEffect(() => {
  ...
  const restaurant = useMemo(() => getRestaurant(id!), [id, refresh])
  const visits = useMemo(() => getVisitsByRestaurant(id!), [id, refresh])

  if (!restaurant) return <div className="text-center py-20 text-stone-400">Restaurant not found.</div>
  ...
  function toggleBestDish(photoId: string, current: boolean) {
    updatePhoto(photoId, { is_best_dish: !current })
    setRefresh(r => r + 1)
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white rounded-2xl p-6 border border-amber-50 shadow-sm mb-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            {restaurant.website && (
              <a href={restaurant.website} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 mt-2 text-amber-600 hover:text-amber-700 text-sm">
                <Globe size={13} /> {restaurant.website}
              </a>
            )}
            {restaurant.phone && (
              <div className="flex items-center gap-1 mt-1 text-stone-400 text-sm">
                <Phone size={13} /> {restaurant.phone}
              </div>
            )}
            {restaurant.description && (
              <p className="text-stone-500 text-sm mt-3 leading-relaxed">{restaurant.description}</p>
            )}
          </div>
          <div className="flex flex-col gap-2 items-end">
            {visits.some(v => v.reviews?.some(r => r.is_pick && r.reviewer === 'sam')) && (
              <span className="bg-blue-100 text-blue-700 text-xs px-3 py-1 rounded-full font-medium">⭐ Sam's Pick</span>
            )}
            {visits.some(v => v.reviews?.some(r => r.is_pick && r.reviewer === 'olivia')) && (
              <span className="bg-pink-100 text-pink-700 text-xs px-3 py-1 rounded-full font-medium">⭐ Olivia's Pick</span>
            )}
            {disagreement && (
              <span className="bg-orange-100 text-orange-700 text-xs px-3 py-1 rounded-full font-medium">🤔 Divided Opinion</span>
            )}
          </div>
        </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {allPhotos.map(photo => (
              <div key={photo.id} className="relative group aspect-square rounded-xl overflow-hidden bg-amber-50">
                ...
                {photo.is_best_dish && (
                  <button
                    onClick={() => toggleBestDish(photo.id, photo.is_best_dish)}
                    ...
                    title="Tap to remove best dish label"
                  >⭐ Best dish ×</button>
                )}
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <Link to={`/add?restaurant=${id}`} className="flex items-center gap-1 bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">
          ...
          {visits.map(visit => (
            <div key={visit.id} className="bg-white rounded-2xl p-5 border border-amber-50 shadow-sm">
              <div className="flex items-start justify-between mb-3">
                ...
                <button onClick={() => handleDeleteVisit(visit.id)} className="text-stone-300 hover:text-red-400 transition-colors">
                  <Trash2 size={15} />
                </button>
              </div>
              {visit.notes && <p className="text-stone-500 text-sm mb-3 italic">"{visit.notes}"</p>}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(['sam', 'olivia'] as const).map(reviewer => {
                  ...
                  if (!review) return (
                    <button
                      key={reviewer}
                      onClick={() => setReviewModal({ visitId: visit.id, reviewer })}
                  ...
                  if (sealed) return (
                    <div key={reviewer} className="bg-stone-100 rounded-xl p-4 text-center border-2 border-dashed border-stone-200">
                      ...
                      <p className="text-xs text-stone-300 mt-0.5">Add yours first, then both reveal</p>
                    </div>
                  )
                  ...
                  return (
                    <div key={reviewer} className="bg-amber-50 rounded-xl p-3">
                      ...
                      <div className="mt-2 text-xs">
                        {{
                          loved:       <span className="text-green-600">😍 Loved it</span>,
                          liked:       <span className="text-green-500">😊 Liked it</span>,
                          indifferent: <span className="text-amber-500">😐 Indifferent</span>,
                          didnt_like:  <span className="text-orange-500">😕 Didn't like it</span>,
                          hated:       <span className="text-red-500">😤 Hated it</span>,
                        }[review.would_return]}
                      </div>
                    </div>
                  )
      ...
      {reviewModal && (
        <AddReviewModal
          ...
          existing={visits
            .find(v => v.id === reviewModal.visitId)
            ?.reviews?.find(r => r.reviewer === reviewModal.reviewer)}
